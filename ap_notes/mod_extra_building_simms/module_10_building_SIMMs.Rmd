---
title: 'Module 10: Building your own SIMM and advanced topics'
...
modelstring ='
model {
  for(i in 1:N) { p[i] ~ dnorm(mu[i],1/pow(sigma,2)) }
  mu <- B%*%beta
...
model {
  ...
  for(l in 2:L) { beta[l] ~ dnorm(beta[l-1],1/pow(sigma_beta,2)) }
