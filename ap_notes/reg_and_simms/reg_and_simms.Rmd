...
- Create a plot:
```{r,include=FALSE}
library(simmr) # to access the demo data files
```
```{r,eval=FALSE}
# Load in the data
...
```{r,fig.align='center', fig.height = 3}
R2jags::traceplot(model_run, varname = 'p_1')
```
...
- There are some useful statistical tests for convergence, including the _Geweke_ test which looks to see whether the mean is stable in each half of the iterations, or the _Brooks, Gelman, Rubin_ (BGR or _Rhat_) test which looks to see whether different chains match. 
- Both `simmr` and `MixSIAR` use the Rhat statistic
- The Rhat test is best if you have multiple chains
