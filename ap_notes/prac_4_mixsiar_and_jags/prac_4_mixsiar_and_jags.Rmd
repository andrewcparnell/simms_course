---
title: 'Practical 4: Using MixSIAR and JAGS'
...
<center><img src="../mod_7_siar_mixsiar/mixsiar_screen.tiff" width="500px" /></center>

At this point you should go back to module 7 ('An introduction to MixSIAR') and run the Wolves example as detailed in slide 17 onward. As part of the model run MixSIAR produces a file `MixSIAR_model.txt` which contains the JAGS script which MixSIAR uses. It's usually a very good idea to have a look at this file to see exactly what model MixSIAR is fitting.
...
Let's start with the simple SIMM defined in Module 9. The model script is as follows:
...
Let's now take the second more advanced SIMM from module 9 and run this on the Geese data. The new JAGS code is:
...
To compare whether the simpler or more complicated SIMM works best, let's compute the DIC (see module 8 lecture notes for details). We can do this with two simple commands. For model 1:
...
1. Associated with module 8 and module 9 are some R scripts which run the harmonic and spline models on the larger Geese data set. Run the fitting and plotting scripts and see if you can understand what is being run. If the JAGS runs are too slow on your computer use my saved version of the output (this has an `.rda` file extension)
