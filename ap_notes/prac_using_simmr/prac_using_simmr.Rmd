...
We will use the data included with simmr. When running your own models you might run into data loading problems. We have lots of examples of data structures from which to load your data. See for example the `vignette("quick_start")` example. At minimum you need two files to get simmr working; a consumers file and a sources file. The simplest geese data set is obtained via:
...
This data set is arranged as a list containing all the consumer/mixture, source, and correction data. The consumer data has two columns, one for each isotope, and 9 individuals. A useful command for learning about the structure of an R data set is `str`, especially for large data objects:

```{r}
str(geese_data_day1$mixtures)
```

We can see that it has 9 rows, 2 columns, and is of numeric (`num`) mode. The two column labels refer to the $\delta^{13}$C and $\delta^{15}$N isotope values. 

The sources data can be obtained from:

```{r}
print(geese_data_day1$source_names)
print(geese_data_day1$source_means)
print(geese_data_day1$source_sds)
```

We can see that there are 4 sources with their names. The remaining matrices refer to the means and standard deviations for each source (row) on each isotope (column). The isotopes need to be in the same order as the consumer data in `mixtures`. 
...
```{r}
print(geese_data_day1$correction_means)
print(geese_data_day1$correction_sds)
```

Lastly, if we have concentration dependence we can include these too. The data for the Geese comes from:

```{r}
print(geese_data_day1$concentration_means)
```
...
The extra `header=TRUE` argument tells R that there are column names at the top of the file. See also the quick start simmr guide that shows you how to load everything in from an Excel file: `vignette("quick_start")`.

***

1. There is an element in the `geese_data_day1` called `tracer_names`. What type is it and what information does it contain? 
2. There's another data object that comes with simmr called `geese_data`. How does the `mixtures` elements of this object differ from `geese_data_day1`.
3. Create some simple scatter plots of the `mixtures` data using `plot`. See if you can add in the source means corrected for the TEF means (hint: add the means together and then plot using `points`)
...
This is a much bigger data set. There is now an element which contains the group number. To run simmr you now need to specify the groups so that it can separate out the model. We can see how many and how large the groups are with:

```{r}
table(geese_data$groups)
```
...
```{r,results='hide', message=FALSE}
simmr_in3 = with(geese_data_day1,
