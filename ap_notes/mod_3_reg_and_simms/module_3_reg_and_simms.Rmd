---
title: 'Module 3: Differences between regression models and SIMMs'
...
plot(x,y,xlab='Percentage of food source in diet (x)',ylab='Weight (y)',las=1)
...
model {
  for(i in 1:N) { 
    y[i] ~ dnorm(alpha + beta*x[i],1/pow(sigma,2)) 
  }
  alpha ~ dnorm(0,0.001) # Note: these are all vague priors
  beta ~ dnorm(0,0.001)
  sigma ~ dunif(0,100)
}
'
...
- When the response variable is binary we use a GLM called _logistic regression_
We can write this new model as:
...
- Another questions (which we will look at on Thursday) is whether this is the 'best' model for the data
...
model=jags.model(textConnection(modelstring), data=data, inits=inits, n.chains=3)
output=coda.samples(model=model,variable.names=c("p_1"),n.iter=10000)
```
