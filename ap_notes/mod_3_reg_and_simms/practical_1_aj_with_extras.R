...
model {
  for(i in 1:N) { 
    ...
    y_pred[i] ~ dnorm( p_1 * s_1 + p_2 * s_2, 1 / pow(sigma, 2) )  
...
output = coda.samples(model = model,
                      variable.names = c("p_1","p_2", "y_pred"),
                      n.iter = 10000)



# plot only the first two posterior distributions which are p1 and p2
plot(output[[1]][,1:2])

# extract the y_pred values
y_pred_posterior <- output[[1]][, 3:11]

y_pred_quantiles = apply(y_pred_posterior, 2, 
                         'quantile',probs=c(0.05,0.95))

y_pred_means = colMeans(y_pred_posterior)

round(cbind(data$y, t(y_pred_quantiles[])),2)

plot(data$y ~ y_pred_means)
