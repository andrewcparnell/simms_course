...
We might focus on d13C first.

```{r}

g_muscle_C <- ggplot(data = mydata,
...
d13C.interaction.model <- glm(Muscle.d13C ~ Species + Length + Species:Length, 
                              data = mydata)
summary(d13C.interaction.model)


```

And now we could consider d15N in a similar manner.

```{r}

g_muscle_N <- ggplot(data = mydata,
                     mapping = aes(x = Length, 
                                   y = Muscle.d15N,
                                   color = Species)) + 
  geom_point() + 
...
d15N.interaction.model <- glm(Muscle.d15N ~ Species + Length + Species:Length, 
                              data = mydata)
summary(d15N.interaction.model)
...
perm.model <- adonis2(Y_muscle ~ Species, 
                     data = mydata, 
                     method = "euclidean", 
                     permutations = 9999)
