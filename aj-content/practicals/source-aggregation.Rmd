...
pp <- posterior_predictive(simmr_out)
```
...
pp_all <- posterior_predictive(simmr_out)
```
...
plot(simmr_in_a_priori) + coord_equal()
```
...
# run the posterior predictive test on this new model
pp_prior <- posterior_predictive(simmr_out_a_priori)

```
And plot the estimated posterior proportions.
...
simmr_out_a_posteriori <- 
  simmr::combine_sources(simmr_out, 
                  to_combine = c("C", "D"), 
                  new_source_name = "CD")
